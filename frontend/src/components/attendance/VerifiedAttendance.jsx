...
export default function VerifiedAttendance() {
  ...
  const verifyAndSubmit = async () => {
    ...
    try {
      ...
      const attendanceData = {
        ...
        address: locationData.address.detailed || 'Location acquired',
        formatted_address: locationData.address.formatted || 'Unknown location',
        location_accuracy: locationData.accuracy,
        location_type: locationData.address.locationType || 'APPROXIMATE',
        place_id: locationData.address.placeId || '',
        // Metadata
        verification_method: 'BIOMETRIC_GPS',
        device_info: navigator.userAgent
      };

      const response = await apiRequest('/api/attendance/verified-checkin', {
        method: 'POST',
      ...
      setTimeout(() => {
        ...
        setLocationVerified(false);
        setSuccess('');
        performFaceScan(); // Auto-start next scan
      }, 3000);
  ...
  const canSubmit = faceVerified && locationVerified && !loading;
