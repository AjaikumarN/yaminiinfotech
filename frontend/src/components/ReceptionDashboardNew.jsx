...
const ReceptionDashboardNew = () => {
  ...
  useEffect(() => {
    if (user) {
      fetchDashboardData();
      fetchBackendNotifications(); // Fetch notifications immediately
      const interval = setInterval(() => {
        fetchDashboardData();
        fetchBackendNotifications(); // Poll notifications every minute
      }, 60000);
  ...
  return (
    <div className="reception-dashboard">
      ...
      <div className="kpi-strip">
        ...
        <div className="kpi-card calls">
          <span className="kpi-icon">📞</span>
          <div className="kpi-content">
            <div className="kpi-label">Calls</div>
            <div className="kpi-value">{kpis.callsMade}/{kpis.callsTarget}</div>
          </div>
        </div>
              ...
              {missingReports.length === 0 ? (
                ...
                missingReports.map((report, idx) => (
                  <tr key={idx}>
                    ...
                    <td>{report.role}</td>
                    <td>{report.missing_item}</td>
                    <td className="warning">{report.days_pending} days</td>
                  </tr>
                ))
              )}
            </tbody>
